...
try:
    sql = "select distinct \"PRONAC\" from versalic_captacao"
    with get_connection().connect() as conn:
        query = conn.execute(text(sql))         
        ja_tem_id_capta = [str(x) for x in pd.DataFrame(query.fetchall())['PRONAC'].to_list()]
    print('len ja_tem_id_capta', len(ja_tem_id_capta))
    id_danca = list(set(id_danca) - set(ja_tem_id_capta))
except:
    pass
...
for PRONAC in id_danca:
    try:
        ...
        dados4 = dados2.json()['_embedded']['captacoes']
        if PRONAC not in ja_tem_id_danca:
            if dados3 != []:
                #print(dados3)
                ...
                print('pronac vazio')
        if PRONAC not in ja_tem_id_capta:
            if dados4 != []:
                for i,x in enumerate(dados4):
                    dados4[i]['incentivador'] = dados4[i]['_links']['incentivador'].split('/')[-1]
                    del dados4[i]['_links']
                df4 = pd.DataFrame(dados4)
                df4.to_sql('versalic_captacao', con= get_connection(), if_exists='append', index=False)
    except:
        print('erro')
    sleep(3)
# %%
